...
uint64_t Factorial(const struct Arguments *args) {
  uint64_t ans = 1;
  // TODO: my code here
...
void *ThreadFactorial(void *args) {
  struct Arguments *fargs = (struct Arguments *)args;
...
int main(int argc, char **argv)
{
  ...
  while (true)
  {
    ...
    while (true)
    {
      ...
      fprintf(stdout, "Receive: from %llu to %llu mod %llu\n", begin, end, mod);

      uint64_t part = (end-begin+1)/tnum;

      struct Arguments args[tnum];
